...
export async function getScorecardCourses() {
  return safe(async () => {
    // Courses that currently have a live scorecard, with hole count + par.
    // Explicit high limit — the default 1000-row cap would truncate ~99×18 holes.
    const { data: cards } = await db().from('scorecards').select('course_id, par').limit(10000);
    if (!cards || !cards.length) return [];
    const byCourse = {};
    for (const s of cards) {
      byCourse[s.course_id] = byCourse[s.course_id] || { holes: 0, par: 0 };
      byCourse[s.course_id].holes++;
      byCourse[s.course_id].par += s.par;
    }
    const ids = Object.keys(byCourse);
    const { data: courses } = await db()
      .from('courses')
      .select('id, slug, name, town, province, country')
      .in('id', ids);
    return (courses || [])
      .map((c) => ({ ...c, ...byCourse[c.id] }))
