...
export function computeBadges({ n, n19, nPhotos, byProvince, provinceTotals, nFirsts = 0 }) {
  // Province badges are a South African pursuit — Mauritius rounds count
  // toward totals but don't create their own "Master" badge.
  const bestProvince = Object.entries(byProvince)
    .filter(([p]) => p !== 'Mauritius')
    .sort((a, b) => b[1] - a[1])[0];
  const badges = [
    ...
    ...Object.entries(provinceTotals || {}).filter(([p]) => p !== 'Mauritius').map(([p, t]) => ({
      name: `${p} Master`,
